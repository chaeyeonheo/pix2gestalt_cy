...
def process_image(
    ...
    """
    단일 이미지에 대한 처리 및 결과 저장 (이미지별 폴더 구조 지원)
    
    Args:
        image_id: 이미지 ID (폴더 이름)
        data_dir: 데이터 디렉토리
    ...
    # 이미지 경로 생성
    item_path = Path(data_dir) / image_id  # 이미지 ID에 해당하는 폴더 경로
    img_path = item_path / f"{image_id}.jpg"
    mask_path = item_path / f"{image_id}_mask.png"
    partial_path = item_path / f"{image_id}_partial.jpg"
...
def run_inference(args):
    ...
    # 이미지 ID 목록 로드 또는 디렉토리에서 자동 탐색
    image_ids = []
    
    if args.input_dir:
        # 디렉토리에서 모든 유효한 이미지 ID 찾기 (이미지별 폴더 구조)
        for item in os.listdir(args.input_dir):
            item_path = Path(args.input_dir) / item
            ...
            if os.path.isdir(item_path):
                ...
                has_comp = False
                for i in range(1, args.n_max_completions + 1):
...
def main():
    ...
    # 데이터 관련 인수
    parser.add_argument('--input_dir', type=str, required=True, help='입력 데이터 디렉토리')
    parser.add_argument('--image_list', type=str, help='처리할 이미지 ID 목록 파일 (지정하지 않으면 디렉토리에서 자동 탐색)')
