...
# 별도 파일로 분리한 데이터셋 모듈 임포트
from amodal_fusion_dataset import AmodalFusionDataset, create_dataloaders, create_distributed_dataloaders
...
def train_fusion_transformer(args):
    """
    Transformer 기반 AmodalFusionNet 훈련 함수 (비분산 훈련)
...
def train_distributed(local_rank, world_size, args):
    ...
    if local_rank == 0:
        print(f"모델 파라미터 수: {sum(p.numel() for p in model.module.parameters() if p.requires_grad):,}")
...
def main():
    ...
    # 데이터 관련 인수
    parser.add_argument('--train_dir', type=str, required=True, help='훈련 데이터셋 디렉토리')
    parser.add_argument('--val_dir', type=str, required=True, help='검증 데이터셋 디렉토리')
    parser.add_argument('--img_size', type=int, default=256, help='이미지 크기')
    parser.add_argument('--n_max_completions', type=int, default=20, help='최대 완성 이미지 수')
